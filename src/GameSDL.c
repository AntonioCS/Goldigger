...
void doRenderMan(GameState* game)
{
    ...
    if (game->man.slowingDown && !game->man.climbing) {
        ...
        SDL_RenderCopyEx(game->renderer, game->sheetTexture, &srcRect_man, &rect_man, 0, NULL, static_cast<SDL_RendererFlip>(game->man.facingLeft));
    ...
    } else if (!game->man.onLedge && !game->man.climbing) // saut
    {
        ...
        SDL_RenderCopyEx(game->renderer, game->sheetTexture,
            &srcRect_man, &rect_man, 0, NULL, static_cast<SDL_RendererFlip>(game->man.facingLeft));
    } else if (game->man.climbing) {
